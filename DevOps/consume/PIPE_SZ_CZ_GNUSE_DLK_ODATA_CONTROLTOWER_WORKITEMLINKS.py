...
# MAGIC 3. Grava os dados na primeira tabela do processo de carga de workItemsLinks
...
SuitePath = '/mnt/Consume/DevOps/Aurora/'+ sourceFile 
DimWorkItemLinks = spark.read.parquet(SuitePath)

# COMMAND ----------

urlgrava = f'jdbc:sqlserver://{host_name}:{port};databaseName={database}'

DimWorkItemLinks.write\
    .format("jdbc")\
    .mode("overwrite")\
    .option("url", urlgrava)\
