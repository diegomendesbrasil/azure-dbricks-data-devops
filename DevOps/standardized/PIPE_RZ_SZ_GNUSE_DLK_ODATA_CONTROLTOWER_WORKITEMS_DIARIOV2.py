...
df_full = df_full\
  ...
  .withColumn('BacklogPriority', df_full.BacklogPriority.cast(IntegerType()))\
  ...
  .withColumn('Custom_TargetDateGAP', df_full.Custom_TargetDateGAP.cast(IntegerType()))\
  ...
  .withColumn('Custom_Executed_Today', df_full.Custom_Executed_Today.cast(DecimalType()))\
  ...
  .withColumn('Custom_PriorityforRelease', df_full.Custom_PriorityforRelease.cast(IntegerType()))\
  .withColumn('Custom_PriorityRank', df_full.Custom_PriorityRank.cast(IntegerType()))\
  ...
  .withColumn('Custom_Scenario_Order', df_full.Custom_Scenario_Order.cast(DecimalType()))\
  ...
  .withColumn('Effort', df_full.Effort.cast(DecimalType()))\
  ...
  .withColumn('Microsoft_VSTS_TCM_TestSuiteTypeId', df_full.Microsoft_VSTS_TCM_TestSuiteTypeId.cast(IntegerType()))\
  ...
  .withColumn('RemainingWork', df_full.RemainingWork.cast(IntegerType()))\
  ...
  .withColumn('TimeCriticality', df_full.TimeCriticality.cast(IntegerType()))\
  ...
  .withColumn('WorkItemId', df_full.WorkItemId.cast(LongType()))\
  .withColumn('WorkItemRevisionSK', df_full.WorkItemRevisionSK.cast(LongType()))\
...
df_diario.write\
    ...
    .mode("overwrite")\
    .option("url", url)\
    .option("dbtable", "dbo.stgWorkItems")\
    .option("user", user)\
...
for i, c in enumerate(df_diario.columns):
  print(i, c, df_diario.select(c).dtypes[0][1])
